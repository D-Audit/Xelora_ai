...
import { getFiles, getWorkflowRuns, getWorkflows, type FileItem, type WorkflowItem, type WorkflowRunItem } from '@/services/workspace';
import { getSubscription, type UsageSummary } from '@/services/billing';
...
export default function DashboardIndexPage() {
  ...
  const [usage, setUsage] = useState<UsageSummary>({ aiActionsUsed: 0, aiActionsLimit: 0, workflowRunsUsed: 0, workflowRunsLimit: 0, resetDate: null });
  const recentFiles = files.slice(0, 4);
  const recentRuns = runs.slice(0, 3);
  const aiUsage = usage.aiActionsLimit === 'unlimited' ? 0 : getUsagePercentage(usage.aiActionsUsed, usage.aiActionsLimit);
  const workflowUsage = usage.workflowRunsLimit === 'unlimited' ? 0 : getUsagePercentage(usage.workflowRunsUsed, usage.workflowRunsLimit);

  useEffect(() => {
    if (isDesktopApp()) router.replace('/dashboard/agent?new=1');
  }, [router]);
  useEffect(() => {
    Promise.all([getFiles(), getWorkflows(), getWorkflowRuns(), getSubscription()]).then(([fileItems, workflowItems, runItems, billing]) => {
      setFiles(fileItems); setWorkflows(workflowItems); setRuns(runItems); setUsage(billing.usage);
    }).catch(() => undefined);
  }, []);
  ...
  return <div className="mx-auto max-w-7xl space-y-6">
    ...
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Card className="p-5"><div className="flex items-center justify-between"><span className="text-sm text-xelora-text-secondary">Files in workspace</span><FileSpreadsheet className="h-4 w-4 text-xelora-green" /></div><p className="mt-3 text-2xl font-semibold text-xelora-text">{files.length}</p><p className="mt-1 text-xs text-xelora-text-muted">{files.filter((file) => file.status === 'needs_review').length} need your review</p></Card><Card className="p-5"><div className="flex items-center justify-between"><span className="text-sm text-xelora-text-secondary">Active workflows</span><Workflow className="h-4 w-4 text-xelora-green" /></div><p className="mt-3 text-2xl font-semibold text-xelora-text">{workflows.filter((workflow) => workflow.status === 'published').length}</p><p className="mt-1 text-xs text-xelora-text-muted">Ready to run when you are</p></Card><Card className="p-5"><div className="flex items-center justify-between"><span className="text-sm text-xelora-text-secondary">AI actions</span><Sparkles className="h-4 w-4 text-xelora-green" /></div><p className="mt-3 text-2xl font-semibold text-xelora-text">{usage.aiActionsUsed.toLocaleString()} <span className="text-sm font-normal text-xelora-text-muted">/ {usage.aiActionsLimit}</span></p><Progress value={aiUsage} className="mt-3 h-1.5 bg-xelora-surface-2" indicatorClassName="bg-xelora-green" /></Card><Card className="p-5"><div className="flex items-center justify-between"><span className="text-sm text-xelora-text-secondary">Workflow runs</span><Play className="h-4 w-4 text-xelora-green" /></div><p className="mt-3 text-2xl font-semibold text-xelora-text">{usage.workflowRunsUsed} <span className="text-sm font-normal text-xelora-text-muted">/ {usage.workflowRunsLimit}</span></p><Progress value={workflowUsage} className="mt-3 h-1.5 bg-xelora-surface-2" indicatorClassName="bg-xelora-green" /></Card></div>
    ...
    <Card><div className="flex items-center justify-between border-b border-xelora-border px-5 py-4"><div><h2 className="font-semibold text-xelora-text">Recent workflow activity</h2><p className="mt-0.5 text-sm text-xelora-text-secondary">The latest work completed in your workspace.</p></div><Link href="/dashboard/history" className="text-sm font-medium text-xelora-green hover:underline">View history</Link></div><div className="divide-y divide-xelora-border">{recentRuns.length ? recentRuns.map((run) => { const status = runStatus[run.status] ?? runStatus.running; const timestamp = run.completedAt ?? run.startedAt; return <Link key={run.id} href={`/dashboard/history/${run.id}`} className="flex flex-col gap-2 px-5 py-4 transition-colors hover:bg-xelora-surface-2 sm:flex-row sm:items-center"><span className="min-w-0 flex-1"><span className="block text-sm font-medium text-xelora-text">{run.workflowName}</span><span className="mt-0.5 block truncate text-xs text-xelora-text-muted">{timestamp ? formatRelativeTime(timestamp) : 'Time unavailable'}</span></span><span className="text-xs text-xelora-text-muted">{run.stepsCompleted}/{run.totalSteps} steps</span><Badge variant={status.variant}>{status.label}</Badge></Link>; }) : <p className="px-5 py-8 text-center text-sm text-xelora-text-muted">No workflow runs yet.</p>}</div></Card>
  </div>;
}
