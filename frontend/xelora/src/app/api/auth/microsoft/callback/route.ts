...
export async function GET(req: NextRequest) {
  ...
  if (errorParam) {
    return NextResponse.redirect(`${origin}/login?error=${encodeURIComponent('Microsoft sign-in was cancelled.')}`);
  }
  if (!code || !state || !expectedState || state !== expectedState) {
    return NextResponse.redirect(
      `${origin}/login?error=${encodeURIComponent('Microsoft sign-in could not be verified. Please try again.')}`
    );
  }

  const redirectUri = `${origin}/api/auth/microsoft/callback`;
  const result = await backendFetch<ExchangeResponse>('/auth/microsoft/exchange', {
    method: 'POST',
